...
# Labo 05 : Microsoft Entra Privileged Identity Management
# Manuel de labo de l’étudiant

## Scénario du labo

Vous avez été invité à créer une preuve de concept qui utilise Microsoft Entra Privileged Identity Management (PIM) pour activer l’administration juste-à-temps et contrôler le nombre d’utilisateurs qui peuvent effectuer des opérations privilégiées. Voici les conditions spécifiques :

- Créer une affectation permanente de l’utilisateur Microsoft Entra ID aaduser2 au rôle Administrateur de sécurité. 
- Configurer l’utilisateur Microsoft Entra ID aaduser2 pour qu’il soit éligible aux rôles Administrateur de facturation et Lecteur global.
- Configurer l’activation du rôle Lecteur global pour demander une approbation de l’utilisateur Microsoft Entra ID aaduser3.
...
> Avant de continuer, vérifiez que vous avez terminé le Labo 04 : MFA, accès conditionnel et Microsoft Entra Identity Protection. Vous aurez besoin à la fois du locataire Microsoft Entra AdatumLab500-04 et des comptes d’utilisateur aaduser1, aaduser2 et aaduser3.
...
## Diagramme de Microsoft Entra Privileged Identity Management
...
Dans cette tâche, vous allez rendre un utilisateur éligible à un rôle Microsoft Entra ID.

1. Connectez-vous au Portail Azure à l’adresse **`https://portal.azure.com/`**.

    >**Remarque** : vérifiez que vous êtes connecté au locataire Microsoft Entra **AdatumLab500-04**. Vous pouvez utiliser le filtre **Annuaire + abonnement** pour basculer d’un locataire Microsoft Entra à l’autre. Vérifiez que vous êtes connecté en tant qu’utilisateur avec le rôle Administrateur général.
...
2. Dans le Portail Azure, dans la zone de texte **Rechercher des ressources, des services et des documents,** située en haut de la page Portail Azure, tapez **Microsoft Entra Privileged Identity Management** et appuyez sur la touche **Entrée**.

3. Dans le volet **Privileged Identity Management**, dans la section **Gérer**, cliquez sur **Rôles Microsoft Entra ID**.
...
1. Dans le Portail Azure, revenez au volet **Privileged Identity Management**, puis cliquez sur **Rôles Microsoft Entra ID**.
...
2. Dans la fenêtre du navigateur InPrivate, accédez au Portail Azure à l’adresse **`https://portal.azure.com/`** et connectez-vous à l’aide du compte d’utilisateur **aaduser2**.
    ...
    >**Remarque** : pour vous connecter, vous devez fournir un nom complet du compte d’utilisateur **aaduser2**, y compris le nom de domaine DNS du locataire Microsoft Entra dont vous avez pris note précédemment dans ce labo. Ce nom d’utilisateur est au format aaduser2@`<your_tenant_name>`.onmicrosoft.com, où `<your_tenant_name>` est l’espace réservé représentant votre nom de locataire Microsoft Entra unique. 

3. Dans le Portail Azure, dans la zone de texte **Rechercher des ressources, des services et des documents,** située en haut de la page Portail Azure, tapez **Microsoft Entra Privileged Identity Management** et appuyez sur la touche **Entrée**.
...
10. Dans le volet **Mes rôles \| Rôles Microsoft Entra ID**, basculez vers l’onglet **Affectations actives**. Vous remarquez que le rôle **Administrateur de la facturation** est **activé**.
...
3. Dans le volet **Mes rôles \| Rôles Microsoft Entra ID**, dans la liste des **affectations éligibles**, dans la ligne affichant le rôle **Lecteur général**, cliquez sur **Activer**. 
...
6. Dans le volet **Mes rôles \| Rôles Microsoft Entra ID**, recherchez le rôle **Administrateur de la sécurité**, puis cliquez sur **Activer**. 
...
9. Revenez dans le volet **Mes rôles \| Rôles Microsoft Entra ID**, cliquez sur l’onglet **Affectations actives** et notez que la liste des **affectations actives** inclut **l’administrateur de la sécurité**, mais pas le rôle **Lecteur général**.
...
11. Dans le navigateur InPrivate, connectez-vous au Portail Azure à **`https://portal.azure.com/`** en tant que **aaduser3**.
    ...
    >**Remarque** : si vous rencontrez des problèmes d’authentification en utilisant l’un des comptes d’utilisateur, vous pouvez vous connecter au locataire Microsoft Entra à l’aide de votre compte d’utilisateur pour réinitialiser leurs mots de passe ou reconfigurer leurs options de connexion.

12. Dans le Portail Azure, accédez à **Microsoft Entra Privileged Identity Management** (dans la zone de texte Rechercher des ressources, des services et des documents en haut de la page Portail Azure, tapez Microsoft Entra Privileged Identity Management et appuyez sur la touche Entrée).
...
14. Dans le volet **Approuver les demandes \| Rôles Microsoft Entra ID**, dans la section **Demandes d’activations de rôle**, cochez la case de l’entrée représentant la demande d’activation de rôle au rôle **Lecteur général** par **aaduser2**.
...
18. Dans le Portail Azure, accédez à **Microsoft Entra Privileged Identity Management** (dans la zone de texte Rechercher des ressources, des services et des documents en haut de la page Portail Azure, tapez Microsoft Entra Privileged Identity Management et appuyez sur la touche Entrée).

19. Dans le volet **Privileged Identity Management \| Démarrage rapide**, dans la section **Tâches**, cliquez sur **Mes rôles**.

20. Dans le volet **Mes rôles \| Rôles Microsoft Entra ID**, cliquez sur l’onglet **Affectations actives** et vérifiez que le rôle Lecteur général est désormais actif.
...
- Tâche 1 : Configurer des alertes de sécurité pour les rôles Microsoft Entra dans PIM
...
4. Dans le volet **Privileged Identity Management \| Démarrage rapide**, dans la section **Gérer**, cliquez sur **Rôles Microsoft Entra**.
...
17. Revenez au volet **Privileged Identity Management**, et, dans la section **Gérer**, cliquez sur **Rôles Microsoft Entra ID**.
...
1. Revenez au volet **Privileged Identity Management**, et, dans la section **Gérer**, cliquez sur **Rôles Microsoft Entra ID**.
...
1. Dans le Portail Azure, définissez le filtre **Annuaire + abonnement** sur le locataire Microsoft Entra associé à l’abonnement Azure dans lequel vous avez déployé la machine virtuelle Azure **az500-04-vm1**.

    >**Remarque** : si vous ne voyez pas votre entrée de locataire Microsoft Entra principale, cliquez sur le lien Changer d’annuaire, sélectionnez votre ligne de locataire principale, puis cliquez sur le bouton Changer.
...
6. De retour dans le Portail Azure, utilisez le filtre **Annuaire + abonnement** pour basculer vers le locataire Microsoft Entra **AdatumLab500-04**.

7. Accédez au volet **AdatumLab500-04 Microsoft Entra** et, dans la section **Gérer**, cliquez sur **Licences**.

8. Dans le volet **Licences** | Vue d’ensemble, cliquez sur **Tous les produits**, cochez la case **Microsoft Entra ID P2** et cliquez dessus pour l’ouvrir.

    >**Remarque** : dans le Labo 4 - Exercice 2 - Tâche 4 **Affecter des licences Microsoft Entra ID P2 aux utilisateurs Microsoft Entra** vous deviez affecter des licences Premium aux utilisateurs **aaduser1, aaduser2 et aaduser3**. Assurez-vous de supprimer ces licences des utilisateurs affectés.

9. Dans le panneau **Microsoft Entra ID P2 - Utilisateurs sous licence**, cochez les cases correspondant aux comptes d’utilisateur auxquels vous avez attribué des licences **Microsoft Entra ID P2**. Cliquez sur **Supprimer la licence** dans le volet supérieur et, lorsque vous êtes invité à confirmer, sélectionnez **Oui**.
...
12. Accédez au volet **AdatumLab500-04 - Vue d’ensemble** du locataire Microsoft Entra, sélectionnez **Gérer les locataires**, puis, dans l’écran suivant, cochez la case en regard d’**AdatumLab500-04**. Enfin, sélectionnez **Supprimer**. Dans le volet **Supprimer le locataire AdatumLab500-04**, sélectionnez le lien **Obtenir l’autorisation de supprimer des ressources Azure**, dans le volet **Propriétés** de Microsoft Entra ID, définissez l’option **Gestion d’accès pour les ressources Azure** sur **Oui**, puis sélectionnez **Enregistrer**.
...
14. Revenez au volet **Supprimer le répertoire AdatumLab500-04**, puis cliquez sur **Supprimer**.
    ...
    >**Remarque** : si vous n’êtes toujours pas en mesure de supprimer le locataire et que vous recevez l’erreur **Supprimer tous les abonnements basés sur une licence**, cela peut être dû à tous les abonnements qui ont été liés au locataire. Ici, la **licence gratuite P2** peut générer l’erreur de validation. La suppression de l’abonnement d’évaluation de la licence P2 à l’aide de l’ID d’administrateur général de l’administrateur M365>> **Vos produits** et du portail **Business Store** résout ce problème. Notez également que la suppression du locataire prend plus de temps. Vérifiez la date d’échéance de l’abonnement, et une fois la période d’évaluation terminée, revenez sur Microsoft Entra ID et tentez de supprimer le locataire.    
